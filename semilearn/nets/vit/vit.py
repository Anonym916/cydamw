...
from .encoding.encoding_aug import EncodingAugmenter
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.encoding_aug = EncodingAugmenter(mode="rope", strength=0.0)
    ...
    def extract(self, x, is_strong=False):
        x = self.patch_embed(x)
        x = torch.cat((self.cls_token.expand(x.shape[0], -1, -1), x), dim=1)
        pos = self.encoding_aug(self.pos_embed, is_strong=is_strong)
    ...
    def forward(self, x, only_fc=False, only_feat=False, is_strong=False, **kwargs):
        """
        Args:
        ...
        x = self.extract(x, is_strong=is_strong)
        if self.global_pool:
