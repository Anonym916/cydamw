...
from semilearn.nets.coord import CoordAugmenter
...
class WideResNet(nn.Module):
    def __init__(self, first_stride, num_classes, depth=28, widen_factor=2, drop_rate=0.0, **kwargs):
        ...
        self.conv1 = nn.Conv2d(3, channels[0], kernel_size=3, stride=1,
                               padding=1, bias=True)
        self.coord_aug = CoordAugmenter(strength=0.0)
    ...
    def extract(self, x, is_strong=False, **kwargs):
        if self.use_coord:
            x = self.coord_aug(x, is_strong=is_strong)
        out = self.conv1(x)
        out = self.block1(out)
        out = self.block2(out)
        out = self.block3(out)
    ...
    def enable_coord(self):
        if self.use_coord:
            return
        new_conv = nn.Conv2d(5, self.conv1.out_channels, kernel_size=3, stride=1,
                             padding=1, bias=True)
        with torch.no_grad():
            new_conv.weight.zero_()
            new_conv.weight[:, :3, :, :] = self.conv1.weight
            if self.conv1.bias is not None and new_conv.bias is not None:
                new_conv.bias.copy_(self.conv1.bias)
        self.conv1 = new_conv
        self.use_coord = True
...
# ---------------------------------------------------------
# Unified WRN builder for SemiLearn (FixMatch expects this)
# ---------------------------------------------------------
def build_wideresnet(depth=28, widen_factor=2, num_classes=10, pretrained=False, pretrained_path=None, **kwargs):
    """
    A unified builder used by semilearn.core.utils.get_net_builder()
    so that wrn can be constructed as:
        build_wideresnet(depth=28, widen_factor=8, num_classes=100)
    """
    model = WideResNet(
        first_stride=1,
        depth=depth,
