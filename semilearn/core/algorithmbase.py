...
class AlgorithmBase:
    ...
    def __init__(
        ...
        **kwargs):
        ...
        self.pseudo_label_noise_ratio = getattr(args, 'pseudo_label_noise_ratio', 0.0)
        self.plabel_error_thresholds = getattr(args, 'plabel_error_thresholds', [])
        ...
        # initialize pseudo-label monitor
        from semilearn.core.utils.pseudo_label_monitor import PseudoLabelMonitor
        self.monitor = PseudoLabelMonitor(self.args, self.save_dir, self.rank, self.world_size)
    ...
    def set_optimizer(self):
        ...
        optimizer = get_optimizer(
            ...
            adopt_decouple=getattr(self.args, "adopt_decouple", False),
        )
    ...
    def set_model(self):
        ...
        if hasattr(model, "encoding_aug"):
            model.encoding_aug.mode = self.args.encoding_aug_type
            model.encoding_aug.strength = self.args.encoding_aug_strength
        if hasattr(model, "enable_coord") and self.args.coord_aug_strength > 0:
            model.enable_coord()
            model.coord_aug.strength = self.args.coord_aug_strength
        return model
    ...
    def set_ema_model(self):
        ...
        if hasattr(ema_model, "enable_coord") and self.args.coord_aug_strength > 0:
            ema_model.enable_coord()
            ema_model.coord_aug.strength = 0.0
        ema_model.load_state_dict(self.model.state_dict())
        if hasattr(ema_model, "encoding_aug"):
            ema_model.encoding_aug.mode = self.args.encoding_aug_type
            ema_model.encoding_aug.strength = 0.0
        return ema_model
    ...
    def train_step(self, idx_lb, x_lb, y_lb, idx_ulb, x_ulb_w, x_ulb_s, y_ulb=None):
        """
    ...
    @torch.no_grad()
    def _compute_plabel_error_rates(self, logits_ulb_w, y_ulb):
        """
        For each threshold in self.plabel_error_thresholds, compute the fraction of
        pseudo-labels whose max_prob >= threshold but whose argmax class != true class.

        Returns a dict  {'train/plabel_err@0.70': float, ...}
        """
        if not self.plabel_error_thresholds or logits_ulb_w is None or y_ulb is None:
            return {}

        probs = torch.softmax(logits_ulb_w.detach().float(), dim=-1)
        max_probs, pred_labels = probs.max(dim=-1)
        y_ulb = y_ulb.to(pred_labels.device)

        result = {}
        for t in self.plabel_error_thresholds:
            mask = max_probs >= t
            n_accepted = mask.sum().item()
            if n_accepted == 0:
                result[f'train/plabel_err@{t:.2f}'] = 0.0
            else:
                n_wrong = (pred_labels[mask] != y_ulb[mask]).sum().item()
                result[f'train/plabel_err@{t:.2f}'] = n_wrong / n_accepted
        return result
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for data_lb, data_ulb in zip(self.loader_dict['train_lb'],
                ...
                # Extract y_ulb before process_batch filters it (train_step may not accept y_ulb)
                y_ulb_raw = data_ulb.get('y_ulb')
                batch_data = self.process_batch(**data_lb, **data_ulb)
                self.out_dict, self.log_dict = self.train_step(**batch_data)

                # Track per-threshold wrong pseudo-label rates
                if self.plabel_error_thresholds and y_ulb_raw is not None:
                    err_rates = self._compute_plabel_error_rates(
                        self.out_dict.get('logits_ulb_w'), y_ulb_raw
                    )
                    self.log_dict.update(err_rates)

                # Update monitor
                self.monitor.global_step = self.it
                self.monitor.epoch = self.epoch
                if self.monitor.enable:
                    # Update pseudo-label stats
                    if 'y_ulb' in batch_data and batch_data['y_ulb'] is not None:
                        logits_ulb_w = self.out_dict.get('logits_ulb_w')
                        if logits_ulb_w is None:
                            with torch.no_grad():
                                logits_ulb_w = self.ema_model(batch_data['x_ulb_w'])['logits']
                        self.monitor.update_pseudo_label_stats(logits_ulb_w, batch_data['y_ulb'])
                    
                    # Probe gradient impact
                    if self.it % self.monitor.grad_probe_interval == 0:
                        grad_stats = self.monitor.probe_gradient_impact(
                            self.model, batch_data['x_ulb_w'], batch_data['x_ulb_s'],
                            batch_data.get('y_ulb'), self.out_dict.get('logits_ulb_w'),
                            self.consistency_loss, self.out_dict.get('mask')
                        )
                    else:
                        grad_stats = {}
                    
                    # Log step stats
                    if self.it % self.monitor.log_interval == 0:
                        self.monitor.log_step(grad_stats)
    ...
    def evaluate(self, eval_dest='eval', out_key='logits', return_logits=False):
        ...
        # Record evaluation for monitor
        if self.monitor.enable and eval_dest == 'eval':
            self.monitor.record_evaluation(eval_dict)
