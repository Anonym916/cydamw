...
class BasicDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.is_ulb = is_ulb
        self.onehot = onehot
        self.include_ulb_targets = include_ulb_targets
        ...
        if self.transform is None:
            ...
            if not self.is_ulb:
                ...
                if self.alg == 'fullysupervised' or self.alg == 'supervised':
                    ...
                    ret = {'idx_ulb': idx, 'x_ulb_w':img_w}
                    if self.include_ulb_targets:
                        ret['y_ulb'] = target
                    return ret
                elif self.alg == 'pimodel' or self.alg == 'meanteacher' or self.alg == 'mixmatch':
                    # NOTE x_ulb_s here is weak augmentation
                    ret = {'idx_ulb': idx, 'x_ulb_w': img_w, 'x_ulb_s': self.transform(img)}
                    if self.include_ulb_targets:
                        ret['y_ulb'] = target
                    return ret
                # elif self.alg == 'sequencematch' or self.alg == 'somematch':
                elif self.alg == 'sequencematch':
                    ret = {'idx_ulb': idx, 'x_ulb_w': img_w, 'x_ulb_m': self.medium_transform(img), 'x_ulb_s': self.strong_transform(img)}
                    if self.include_ulb_targets:
                        ret['y_ulb'] = target
                    return ret
                    ...
                    ret = {'idx_ulb': idx, 'x_ulb_w': img_w, 'x_ulb_s_0': img_s1, 'x_ulb_s_1':img_s2, 'x_ulb_s_0_rot':img_s1_rot, 'rot_v':rotate_v_list.index(rotate_v1)}
                    if self.include_ulb_targets:
                        ret['y_ulb'] = target
                    return ret
                elif self.alg == 'comatch':
                    ret = {'idx_ulb': idx, 'x_ulb_w': img_w, 'x_ulb_s_0': self.strong_transform(img), 'x_ulb_s_1':self.strong_transform(img)}
                    if self.include_ulb_targets:
                        ret['y_ulb'] = target
                    return ret
                else:
                    ret = {'idx_ulb': idx, 'x_ulb_w': img_w, 'x_ulb_s': self.strong_transform(img)}
                    if self.include_ulb_targets:
                        ret['y_ulb'] = target
                    return ret 
